...
- (BOOL)isViewControllerTransiting {
    return [[self.navigationController valueForKey:@"_isTransitioning"] boolValue];
}
...
- (instancetype)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if (self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil]) {
        ...
        self.toolbar.tintColor = [UIToolbar appearance].tintColor;
    }
    return self;
}
...
- (void)pushViewController:(UIViewController *)viewController animated:(BOOL)animated {
    if (!viewController) {
        NSAssert(NO, @"%s, self.viewControllers = %@", __func__, self.viewControllers);
        return;
    }
